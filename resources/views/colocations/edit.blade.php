...
<head>
    ...
    <title>Edit Colocation</title>
    <script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="bg-gray-100 min-h-screen flex items-center justify-center">

    <div class="bg-white p-8 rounded-2xl shadow-lg w-full max-w-md">
        <h1 class="text-2xl font-bold mb-6 text-gray-800">Edit Colocation</h1>

        <form action="{{ route('colocation.update', $colocation) }}" method="POST" class="space-y-4">
            @csrf
            @method('PUT')

            <div>
                <label for="name" class="block text-sm font-medium text-gray-700 mb-1">Colocation Name</label>
                <input type="text" name="name" id="name" value="{{ $colocation->name }}"
